...
function ooMutateLocked_(request, requestNowMs) {
  ...
  var identity = ooValidateConversionIdentityOwnership_(loaded.store, request, canonical); if (!identity.ok) return identity;
...
Add a store-wide conversion-identity index over every live **and archived** inspection. Each non-null `conversionId`, `pendingOrderId`, `linkedOrderId`, and `conversionReceiptId` is reserved against every other inspection regardless of which of those four fields holds it; `pendingOrderId===linkedOrderId` is permitted only within its owning inspection after local commit. Begin additionally requires its incoming `conversionId`, `pendingOrderId`, and `receiptId` to be pairwise distinct. A collision returns `conversion-identity-conflict` after exactly one dispatcher request-clock snapshot and the required exact source/store Drive read, with zero mutation-clock snapshots, backup/manifest create-or-trash calls, property writes, audit additions, revision changes, source writes, or retention calls. RED covers each of the four stored fields against each of the three begin inputs for both live and archived owners, same-inspection legal pending/link ownership, record adopting an ID owned by another row, finalize revalidation, and strict-store rejection of pre-existing cross-inspection collisions.

Split concurrent begin precedence into two tests. With two valid requests carrying the same current `expectedRevision`, the first succeeds and increments revision; the second performs the required source/store Drive read and returns `revision-conflict` before identity ownership validation or mutation preparation, with exactly one request-clock snapshot, zero mutation-clock snapshots, and zero backup/manifest create-or-trash calls, property writes, audit additions, revision changes, source writes, or retention calls. Then send a fresh request using the latest revision but reusing any reserved conversion, pending-order, or receipt identity: after the same required source/store read it returns `conversion-identity-conflict` with exactly one request-clock snapshot, zero mutation-clock snapshots, and the same exact zero write-effect counters. This test must fail if identity checking moves ahead of the locked revision comparison.

Add explicit replay REDs for begin, arm, record, and finalize at only the immediately reached stage. The successful audit row must uniquely bind action, record ID, mutation payload hash, `preMutationRevision`, and exact four-key ACK reconstruction; missing, duplicate, inconsistent, or extra audit/ACK data is `invalid-store`. A fresh mutation ID plus the exact proof and pre-success revision returns the byte-identical prior ACK after exactly one request-clock snapshot and the required source/store Drive read, with zero mutation-clock snapshots, backup/manifest create-or-trash calls, property writes, audit additions, revision changes, source writes, or retention calls. Changed receipt/approval proof is `receipt-mismatch`, changed terms/hash is `terms-mismatch`, changed identity or a later-than-immediate stage is `invalid-conversion-state`; none writes. Cancel is deliberately not replay-successful: retry after a successful cancel returns `invalid-conversion-state` and leaves proposal bytes/revision unchanged.
