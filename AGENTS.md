...
운영 기준선은 main 의 최신 버전(2026-09-07 현재 `hyeonjang-v266-aptunits`)이며 전체 회귀는
`tests/run-all.js`가 집계하는 121개다(후속 후보의 추가 검사는 러너 마지막 줄의 집계를 믿어라). 사진 동기화 로컬 후보
...
후속 로컬 후보 `hyeonjang-v267-estimateinfo`: 견적서 목록의 날짜 출처 및 프로젝트 지정 화면.
...
실제 운영 데이터 수정/서버 변경/배포는 하지 않았다. 신규 통합 9/9·변이 3종 검증,
