...
운영 기준선은 main 의 최신 버전(2026-09-07 현재 `hyeonjang-v264-photoguard`)이며 전체 회귀는
`tests/run-all.js`가 집계하는 119개다(후속 후보의 추가 검사는 러너 마지막 줄의 집계를 믿어라). 사진 동기화 로컬 후보
...
후속 후보 `hyeonjang-v265-photosource`는 사진 도구의 읽기 전용 보관 상태 화면을 추가한다.
서버 연결 기록, 현재 화면 파일, PC 파일 연결, 미리보기 기록을 구분하며 원본 백업 성공으로 단정하지 않는다.
서버/저장 구조/사진 배정은 변경하지 않는다. 별도 배포 승인 전 미배포다.
