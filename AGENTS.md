...
> 2026-09-06 기준. 낡은 내용을 발견하면 **이 문서부터 고쳐라.**
...
운영 기준선은 main 의 최신 버전(2026-09-06 현재 `hyeonjang-v261-orderflow`)이며 전체 회귀는
`tests/run-all.js`가 집계하는 116개다(파일 수는 늘어난다 — 러너 마지막 줄의 집계를 믿어라). 사진 동기화 로컬 후보
`hyeonjang-v201-photosyncp0`는 서버 선행 게이트 전에는 계속 병합·배포하지 않는다.
`sw.js`의 캐시 이름이 곧 버전이다.

2026-09-06 현장 검색 개선 후보: `hyeonjang-v262-searchrecovery`는 보관 현장 검색과
사진 검색 결과 없음에서의 복귀를 개선한다. 검색·화면 상태만 변경하며 사진·프로젝트
저장 구조와 서버는 바꾸지 않는다. 별도 배포 승인 전에는 운영 v261과 구분한다.
