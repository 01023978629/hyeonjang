...
운영 기준선은 main 의 최신 버전(2026-09-07 현재 `hyeonjang-v265-photosource`)이며 전체 회귀는
`tests/run-all.js`가 집계하는 120개다(후속 후보의 추가 검사는 러너 마지막 줄의 집계를 믿어라). 사진 동기화 로컬 후보
...
후속 로컬 후보 `hyeonjang-v266-aptunits`: 대표 요청의 아파트 프로젝트 내 동·호수/공용부 관리.
project.aptUnits와 파일의 선택적 aptUnit만 확장하며 기존 프로젝트/사진을 자동 병합·배정하지 않는다.
실데이터 변경과 공개 배포는 아직 하지 않았다. 세대 등록·사진 배정은 안전 스냅샷과 원자 저장을 거친다.
