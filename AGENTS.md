...
운영 기준선은 main 의 최신 버전(2026-09-06 현재 `hyeonjang-v260-refreshflush`)이며 전체 회귀는
`tests/run-all.js`가 집계하는 113개다(파일 수는 늘어난다 — 러너 마지막 줄의 집계를 믿어라). 사진 동기화 로컬 후보
...
# 3) 전체 회귀 — 권위 있는 집계는 tests/run-all.js 가 찍는 'N개 중 N 통과' (2026-09-06 기준 113개)
...
- 현재 전체 회귀 기준은 `tests/run-all.js`가 집계하는 파일 전부(2026-09-06 현재 113개)다. 과거의 브라우저·정적 60개 집계는
