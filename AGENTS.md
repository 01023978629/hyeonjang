...
운영 기준선은 main 의 최신 버전(2026-09-07 현재 `hyeonjang-v263-photointake`)이며 전체 회귀는
`tests/run-all.js`가 집계하는 118개다(후속 후보의 추가 검사는 러너 마지막 줄의 집계를 믿어라). 사진 동기화 로컬 후보
...
후속 로컬 후보 `hyeonjang-v264-photoguard`는 사진 추가와 앱 업데이트의 상호 보호,
기존 작업명/공정명 검색과 통합 검색 사진 결과 이동을 보완한다. 저장 구조·서버는 변경하지 않으며,
아파트 오더의 사진 연결은 작업명 검색 확장과 분리해 기존 기준을 유지한다. 별도 배포 승인 전 미배포다.
