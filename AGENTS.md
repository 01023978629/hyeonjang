...
실제 운영 기준선은 마지막 성공 배포 `hyeonjang-v267-estimateinfo`다. 2026-09-07
main `7cd0ae3`에는 v271(PR #115)이 병합됐으나 Pages 실행 34130259962는
서류 메타데이터 가로 넘침 검사에서 125/126으로 실패하여 공개되지 않았다.
후속 v272는 해당 줄바꿈만 보완하는 배포 수정 후보이며 전체 회귀는
`tests/run-all.js`가 집계하는 126개다. 이번 v273 로컬 후보는 신규 모바일 연결 검사를
...
검증 뒤 v272와 통합했으며 최종 전체 회귀는 별도 기록한다. 배포는 별도 승인 전이다.
...
PR #116 병합 및 공개 배포 확인 전이다. 근거는 `docs/웹앱_테스트결과_리포트_v272.md`.
