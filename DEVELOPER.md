...
- `hjSnapshot(label, force)` — 3분 스로틀. (v261) 12칸 정리는 routine 라벨('작업 중'·'자동'·빈 값) 중 오래된 것부터 버리고, routine 이 없을 때만 강제 스냅샷('사진 삭제 전' 등) 중 가장 오래된 것을 버린다 — 방금 넣은 것은 안 버림. `🤖` 라벨은 **직전 스냅샷도 🤖일 때만** 45초 묶음(연속 AI작업 폭주 방지, 사람 작업 뒤엔 항상 남음).
