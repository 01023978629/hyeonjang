...
(async () => {
  ...
  await scenario('세대별 사진 추가·아파트 변경 시 세대 선택 초기화·실제 PNG 연결 저장', async test => {
    ...
    assert.equal(await page.locator('#photoIntakeProject option:checked').textContent(), A, 'photo-zero-first sorting preserves the selected apartment');
    assert.equal(await page.locator('#photoIntakeUnit').inputValue(), 'unit-shared', 'entry from unit card preselects that unit');
    const before = await snapshot(page);
    await page.locator('#photoIntakeProject').selectOption({ label: B });
    ...
    await page.locator('#photoIntakeProject').selectOption({ label: '기존 일반현장' });
    ...
    await page.locator('#photoIntakeProject').selectOption({ label: A });
