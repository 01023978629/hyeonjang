...
(async () => {
  ...
  assert(val(r, '벽지').startsWith('3 롤') && val(r, '벽 폭 수') === '16 폭 (롤당 7폭, 여유 10cm)' && val(r, '면적법 참고').startsWith('2 롤') && val(r, '벽 면적') === '28.2 ㎡' && val(r, '도배 평') === '8.5 평', '② 도배 폭 재단: ' + JSON.stringify(r));
  ...
  assert(val(r, '전류') === '17 A (220V, 역률 0.8)' && val(r, '차단기 참고') === '30 A', '② 전기 역률: ' + JSON.stringify(r));
  ...
  assert(val(r, '석고보드') === '40 장' && val(r, '피스 참고') === '1,280 개 (장당 32개 기준)', '② 석고보드 2겹: ' + JSON.stringify(r));
  ...
  assert(val(r, '각재 (간격 450mm 한 방향 어림)').startsWith('7 본') && /한 방향/.test(r.note) && val(r, '합판').startsWith('4 장'), '② 목재 간격 어림: ' + JSON.stringify(r));
  ...
  assert(await page.evaluate(() => { const o = document.querySelector('#calcOut'); const cs = getComputedStyle(o); return cs.position === 'sticky' && o.getAttribute('aria-live') === 'polite' && o.getAttribute('role') === 'status'; }), '③ 결과 상자는 스크롤해도 붙어 있고 소리로 읽힌다');
  ...
  p = await prefs();
  assert(Object.keys(p.d).length === 0 && /저장한 기본값이 없습니다/.test(await modalText()), '⑧ 전부 처음으로');
  ...
  assert(log.length >= 3 && log.length <= 20 && tileLogs.length >= 1 && /타일 \(로스 포함\) 73 장/.test(log.find(e => e.cat === 'tile').head), '⑩ 계산마다 기록, 화면 하나에 한 건: ' + JSON.stringify(log.map(e => [e.cat, e.head])));
  ...
  assert((await logRead()).length === before && (await page.evaluate(() => document.querySelectorAll('#modalRoot .calcLogRow').length)) === before, '⑩ ✕ 하나 지움');
  await page.click('#modalRoot .mfoot button:has-text("모두 지우기")');
  assert((await logRead()).length === 0 && /아직 기록이 없습니다/.test(await modalText()), '⑩ 모두 지우기');
  await page.evaluate(() => materialCalcSettings());
