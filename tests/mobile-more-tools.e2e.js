/* v269: fake local state only. Every menu endpoint is stubbed before the 115-action dispatch sweep.
...
const TARGETS={
  ...
  notify:['notifySettings'],expense:['expenseLedger'],pnl:['pnl'],search:['hjGlobalSearch'],materials:['materialCatalog'],calc:['materialCalc'],measure:['measureNote'],todo:['todoView'],suppliers:['suppliers'],annual:['annualReport'],
...
assert.equal(Object.keys(TARGETS).length,115,'expected action contract contains all 115 registered actions');
...
(async()=>{browser=await chromium.launch({headless:true});
  await run('115개 메뉴의 실제 클릭→dispatch→함수·인자 일치',async t=>{
    ...
    const records=await t.page.evaluate(async targets=>{
      ...
      if(new Set(actions).size!==115||JSON.stringify([...actions].sort())!==JSON.stringify(Object.keys(targets).sort()))throw new Error('catalog differs from independent 115-action contract');
    ...
    assert.equal(records.length,115);for(const r of records){const [fn,...args]=TARGETS[r.action];assert.deepEqual(r.hits,[{fn,args}],'exact endpoint for '+r.action);assert.equal(r.result,true,'registered successful dispatch '+r.action);assert(r.enabled&&r.width>=43.5&&r.height>=43.5,'enabled 44px menu chip '+r.action);if(r.action==='ledgertab')assert.equal(r.tab,'ledger');if(r.action==='adstab')assert.equal(r.tab,'ads');if(['ledgertab','adstab'].includes(r.action))assert.equal(r.project,null);if(r.action==='addproject')assert.equal(r.newProject,'가상 신규 현장');}
  ...
  console.log('== mobile-more-tools: '+passed+' passed; 115 action clicks checked; pageerrors=0 ==');await browser.close();
