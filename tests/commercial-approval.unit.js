...
assert.equal(sandbox.caCanonicalTerms_({ ...terms.value, scope: '한글 UTF-8 SHA-256' }).sha256Hex,
  crypto.createHash('sha256').update('{"workKind":"device-diagnosis","scope":"한글 UTF-8 SHA-256","exclusions":["복구 공사","타일"],"vatMode":"included","quotedAmount":100000,"validUntil":"2026-09-30","scheduleWindow":"2026-09-02 오후"}').digest('hex'));
...
assert.equal(sandbox.caValidateNonce_('a'.repeat(16)), true);
assert.equal(sandbox.caValidateNonce_('z'.repeat(80)), true);
assert.equal(sandbox.caValidateNonce_('a'.repeat(15)), false);
assert.equal(sandbox.caValidateNonce_('z'.repeat(81)), false);
assert.equal(sandbox.caValidateNonce_('nonce_AB-12_cd_xx'), true);
assert.equal(sandbox.caValidateNonce_('nonce.with.dot_123'), false);
...
const canonicalReceipt = '{"receiptId":"receipt_test_1","subjectType":"aptOrder","subjectId":"pending_order_test_1","approvedTermsSha256":"' + terms.sha256Hex + '","approvalEvidenceType":"quote-file","approvalEvidenceFileId":"TEST_EVIDENCE_FILE_0001","approvalEvidenceSha256":"' + 'a'.repeat(64) + '","approvedAt":"2026-08-31T09:30:00+09:00","approvedByRole":"customer","issuedAt":"2026-08-31T10:00:00+09:00"}';
assert.equal(sandbox.caReceiptCanonical_(receipt), canonicalReceipt);
assert.equal(sandbox.caReceiptCanonical_({ ...receipt, receiptHmac: 'changed-but-excluded' }), canonicalReceipt);
assert.equal(receipt.receiptHmac, crypto.createHmac('sha256', 'TEST_ONLY_RECEIPT_HMAC_KEY').update(canonicalReceipt).digest('hex'));
...
for (const [approvalEvidenceType, approvedByRole] of [['contract-file', 'customer'], ['message-export-file', 'management-office']]) {
  const alternate = { ...receipt, approvalEvidenceType, approvedByRole };
  alternate.receiptHmac = crypto.createHmac('sha256', 'TEST_ONLY_RECEIPT_HMAC_KEY')
    .update(sandbox.caReceiptCanonical_(alternate)).digest('hex');
  assert.equal(sandbox.caVerifyReceiptMac_(alternate, 'TEST_ONLY_RECEIPT_HMAC_KEY'), true);
}
for (const invalidChanges of [{ approvalEvidenceType: 'invoice-file' }, { approvedByRole: 'staff' }]) {
  const invalid = { ...receipt, ...invalidChanges };
  invalid.receiptHmac = crypto.createHmac('sha256', 'TEST_ONLY_RECEIPT_HMAC_KEY')
    .update(sandbox.caReceiptCanonical_(invalid)).digest('hex');
  assert.equal(sandbox.caVerifyReceiptMac_(invalid, 'TEST_ONLY_RECEIPT_HMAC_KEY'), false);
}
