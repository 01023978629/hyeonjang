...
(async () => {
  ...
  await test('직렬화 왕복 — projects/notes/files 보존·최상위 키 불변·신규키 금지', async () => {
    ...
    const ALLOWED = ['version', 'app', 'savedAt', 'learn', 'quotes', 'schedule', 'calendarImports', 'notes', 'priceBook', 'asLog', 'aptOffices', 'aptOrders', 'officeIntake', 'aptRates', 'monthClosed', 'quoteSets', 'trips', 'tripCfg', 'workLogs', 'claudeDone', 'satisfaction', 'adPosts', 'portalCfg', '_savedFileCount', 'kakaoLastAt', 'coworkTasks', 'coworkSched', '_cwSchedInit', '_coworkInit', 'payLog', 'expenses', 'goals', 'aiOps', 'suppliers', 'supplierMap', 'inventory', 'materials', 'brand', 'contacts', 'projects', 'files'].sort();
