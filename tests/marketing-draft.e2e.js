...
(async () => {
  ...
  await test('새 최상위 직렬화 키 없음 + project 파생값 오염 없음', async () => {
    ...
    const allowed = new Set(['version','app','savedAt','learn','quotes','schedule','calendarImports','notes','priceBook','asLog','aptOffices','aptOrders','officeIntake','aptRates','monthClosed','quoteSets','trips','tripCfg','workLogs','claudeDone','satisfaction','adPosts','portalCfg','_savedFileCount','kakaoLastAt','coworkTasks','coworkSched','_cwSchedInit','_coworkInit','payLog','expenses','goals','aiOps','suppliers','supplierMap','inventory','brand','contacts','projects','files']);
