...
const required = ['normalizeCommercialTerms','commercialNow','validateCommercialApproval','issueCommercialApproval','normalizeReceipt','officeIntakeSnapshot','durableLocalMutation','durablePaidMutation','paidCommitWriteAtomic','readPaidCommitSnapshot','guardedPersistCurrentState','resolvePaidCommitState','persistApprovedAptOrder','persistGatedAptTransition','executePaidWorkGate','transitionAptOrderWithGate','updateCommercialTerms'];
...
(async()=>{
  const posts=[],transport={Date,JSON,Object,Error,fetch:async(url,init)=>{posts.push({url,init:{...init,body:JSON.parse(init.body)}});return{ok:true,json:async()=>({ok:true,receiptId:'receipt_transport'})};}};vm.createContext(transport);vm.runInContext("const __commercialApproval={url:'https://commercial.example/exec',token:'approval-token'};",transport);for(const name of ['commercialEnvelope','postIsolated','commercialError','commercialCall'])vm.runInContext(extractFunction(name),transport);await transport.commercialCall('commercialNow',{nonce:'transport_nonce'});assert.deepEqual(Object.keys(posts[0].init.body),['token','action','timestamp','payload']);for(const forbidden of ['deviceId','mutationId','ts'])assert.equal(Object.hasOwn(posts[0].init.body,forbidden),false,'commercial envelope excludes '+forbidden);
  ...
  assert.equal(value,'ok');assert.deepEqual(order,['snapshot:true','deadline','validate','deadline','transaction','consume','apply','render']);assert.equal(committed.aptOrders.length,1);assert.equal(consumed,1);
