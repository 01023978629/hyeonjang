...
2. 권한 승인 창이 뜨면 5번과 동일하게 **허용** (메일 발송 권한이 추가로 필요합니다)
3. 실행 로그에 `설치 완료: 매일 7시경 → (내 메일주소)` 가 보이면 성공
4. **바로 확인해보기** — 함수 선택을 **`testWatchNow`** 로 바꾸고 실행하면 지금 즉시 메일이 옵니다

이제 매일 아침 7시경, 챙길 게 있으면 메일이 옵니다. 조용한 날은 안 보내고 **월요일에만** "이상 없음" 확인 메일이 옵니다.

### 파수꾼 설정 바꾸기 (선택 · 스크립트 속성)

| 속성 | 뜻 |
|---|---|
| `WATCH_EMAIL` | 받을 메일 주소 (미설정 시 이 계정 메일) |
| `WATCH_HOUR` | 발송 시각 0~23 (기본 `7`) — 바꾼 뒤 `installDailyWatch` 를 다시 실행 |
| `WATCH_OFF` | `1` 이면 발송 중단 |
